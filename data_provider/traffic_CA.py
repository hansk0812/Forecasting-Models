...
class Dataset_PEMS_Traffic(Dataset):
    ...
    def __read_data__(self):
        ...
        with open(fname, 'rb') as f:
            df_raw = np.load(f)["data"]
        df_raw = df_raw.reshape((df_raw.shape[0], -1))
        
        self.num_features = df_raw.shape[-1]
        df_raw = pd.DataFrame(df_raw, columns=['%d' % idx for idx in range(self.num_features)])
        
        if "PEMS03" in self.data_path:
            start_date = datetime.strptime("01/09/2018", "%d/%m/%Y")
        elif "PEMS04" in self.data_path:
            start_date = datetime.strptime("01/01/2018", "%d/%m/%Y")
        elif "PEMS07" in self.data_path:
            start_date = datetime.strptime("01/05/2017", "%d/%m/%Y")
        else:
            start_date = datetime.strptime("01/07/2016", "%d/%m/%Y")
