...
class Dataset_Weather_Stations_ALL(Dataset):
    ...
    def __init__(self, root_path, flag='train', size=None,
        ...
        self.cycle_index = np.array([idx % cycle for idx in range(len(self.timestamp))]).astype(np.int32)
    ...
    def __getitem__(self, indexs):
        data, cycle = self.batch_sample(indexs)
        ...
        print (seq_x.shape); exit()
        if self.flag=='test':
            percentile = self.percentiles[:(seq_x.shape[0]//350)*350,:,:][:,None,:,:]
            return (seq_x, percentile), seq_y, seq_x_mark, seq_y_mark, cycle
    ...
    def batch_sample(self, indexs): 
        ...
        cycle_x = self.cycle_index[s_begin + self.seq_len]
