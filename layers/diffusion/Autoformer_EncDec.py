...
class moving_avg(nn.Module):
    ...
    def __init__(self, kernel_size, stride):
        ...
        self.kernel_size = kernel_size if not isinstance(kernel_size, list) else kernel_size[0]
        self.avg = nn.AvgPool1d(kernel_size=kernel_size, stride=stride, padding=0)

    def forward(self, x):
        # padding on the both ends of time series
        rep = (self.kernel_size - 1) // 2 if self.kernel_size % 2 == 1 else self.kernel_size // 2
        front = x[:, 0:1, :].repeat(1, rep if self.kernel_size % 2 == 1 else rep-1, 1)
        end = x[:, -1:, :].repeat(1, rep, 1)
