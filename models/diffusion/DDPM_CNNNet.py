...
class InputConvNetwork(nn.Module):
    ...
    def __init__(self, args, inp_num_channel, out_num_channel, num_layers=3, ddpm_channels_conv=128):
...
class My_DiffusionUnet_v0(nn.Module):
    ...
    def __init__(self, args, device, num_vars, seq_len, pred_len, net_id=0, smoothed_factors=None):
        ...
        self.net_id = net_id
        self.smoothed_factors = smoothed_factors #[int(x) for x in args.detail_freq[1:-1].split(',')] 
        self.num_bridges = len(self.smoothed_factors) + 1

        self.dim_diff_step = args.factor # 256
        time_embed_dim = self.dim_diff_step
        self.diffusion_embedding = DiffusionEmbedding(
            num_steps = 100, #args.diff_steps,
            embedding_dim=self.dim_diff_step,
        )
        self.act = lambda x: x * torch.sigmoid(x)
        
        self.use_features_proj = False
        self.channels = 64 #args.ddpm_inp_embed
        args.ddpm_channels_conv = args.d_model // 2 #128
        if self.use_features_proj:
            self.feature_projection = nn.Sequential(
                                        linear(self.num_vars, self.channels),
                                    )
            self.input_projection = InputConvNetwork(args, self.channels, self.channels, num_layers=10) #args.ddpm_layers_inp)
        else:
            self.input_projection = InputConvNetwork(args, self.num_vars, self.channels, num_layers=10) #args.ddpm_layers_inp)

        self.dim_intermediate_enc = args.d_model #256 #args.ddpm_channels_fusion_I
        self.enc_conv = InputConvNetwork(args, self.channels+self.dim_diff_step, self.dim_intermediate_enc, num_layers=5) #args.ddpm_layers_I)
        
        if True: #self.args.individual: - DLinear channel independence True
            self.cond_projections = nn.ModuleList()
        
        args.ablation_study_F_type = "Linear"
        if args.ablation_study_F_type == "Linear":
            if True: #self.args.individual: - DLinear channel independence
        ...
        if self.net_id == self.num_bridges-1:
            if False: #args.use_ar_init:
                self.combine_conv = InputConvNetwork(args, self.dim_intermediate_enc+2*self.num_vars, self.num_vars, 
                                        num_layers=10, ddpm_channels_conv=256) #args.ddpm_layers_II, args.dec_channel_nums,)
            else:
                self.combine_conv = InputConvNetwork(args, self.dim_intermediate_enc+1*self.num_vars, self.num_vars, 
                                        num_layers=10, ddpm_channels_conv=256) #args.ddpm_layers_II, args.dec_channel_nums,)
        else:
            if False: #args.use_ar_init:
                self.combine_conv = InputConvNetwork(args, self.dim_intermediate_enc+3*self.num_vars, self.num_vars, num_layers=10, ddpm_channels_conv=args.d_model) #256) 
                                                        #num_layers=args.ddpm_layers_II, ddpm_channels_conv=args.dec_channel_nums)
            else:
                self.combine_conv = InputConvNetwork(args, self.dim_intermediate_enc+2*self.num_vars, self.num_vars, num_layers=10, ddpm_channels_conv=args.d_model) #256)
                                                        #num_layers=args.ddpm_layers_II, ddpm_channels_conv=args.dec_channel_nums)
    ...
    def forward(self, xt, timesteps, cond=None, ar_init=None, future_gt=None, mask=None):
        ...
        self.args.use_future_mixup = True
        self.args.ablation_study_masking_type = "none"
        if self.args.use_future_mixup and future_gt is not None:
            
            y_clean = future_gt[:,:,-self.pred_len:]
            if False: #self.args.beta_dist_alpha > 0:
                rand_for_mask = np.random.beta(self.args.beta_dist_alpha, self.args.beta_dist_alpha, size=np.shape(y_clean))
        ...
        # ar_init = None
        if False: #self.args.use_ar_init:
            if self.net_id == self.num_bridges-1:
        ...
        if True: #self.args.use_residual:
