...
class Diffusion_Worker(nn.Module):
    ...
    def __init__(self, args, device, u_net=None):
        ...
        self.parameterization = "x_start" #args.parameterization
        assert self.parameterization in ["noise", "x_start"], 'currently only supporting "eps" and "x0"'
        
        self.net = u_net

        self.diff_train_steps = args.modes #100
        self.diff_test_steps = self.diff_train_steps

        # self.beta_start = 1e-4 # 1e4
        # self.beta_end = 2e-2

        self.beta_start = float(args.detail_freq[1:].split(',')[0]) # 1e4
        self.beta_end = float(args.detail_freq[1:-1].split(',')[1])
    ...
    def set_backward_pass_inspect_timestep(self, timestep):
        self.backward_pass_inspect_cutoff = timestep

    def get_backward_pass_inspect_timestep(self):
