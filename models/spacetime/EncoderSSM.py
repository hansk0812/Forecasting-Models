...
from .SSMBase import SSMBase, krylov_logarithmic_products

class EncoderSSM(SSMBase):
    ...
    def init_weights(self):
        ...
        self._fp = (self.num_kernels, self.kernel_dim)
        
        # Shift matrix initialization
        self.shift_matrix = torch.zeros(self.num_kernels, 
                                        self.kernel_dim, 
                                        self.kernel_dim)
        ...
        if torch.cuda.is_available():
            self.shift_matrix = self.shift_matrix.to(torch.device("cuda"))
            self.p_padding = self.p_padding.to(torch.device("cuda"))
        
        # A matrix
        a = self.init_kernel_weights(self.kernel_init)
        self.register("a", a, trainable=True)
        
        # B matrix
        b = self.init_kernel_weights(self.kernel_init) 
        self.register("b", b, trainable=True)
        
        # C matrix
        c = self.init_kernel_weights(self.kernel_init)
        self.register("c", c, trainable=True)
    ...
    def norm(self, x, ord=1):
        ...
        # (heuristicky, but we norm for stability) - similar to gradient clipping
        try:
    ...
    def matrix_power(self, l, c, b, p):
        ...
        g = krylov_logarithmic_products(l, A, b, c)
        return g
    ...
    def get_kernel(self, u, c=None, l=None):
        ...
        f = self.matrix_power(l, c, self.b, a)
        return f
