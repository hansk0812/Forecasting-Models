...
from .diffusion.DDPM_CNNNet import *
from .diffusion.DDPM_diffusion_worker import *

from layers.RevIN import *

from .diffusion.samplers.dpm_sampler import DPMSolverSampler

# The loss function is left inside the model in order to prevent returning intermediate states 
# as in the code base: https://github.com/dlgudwn1219/mrDiff
class BaseMapping(nn.Module):
    """
    Decomposition-Linear
    """
    def __init__(self, args, device, seq_len=None, pred_len=None):
        ...
        self.individual = True #args.individual - use DLinear by default
        ...
        self.rev = RevIN(args, args.enc_in, affine=False, subtract_last=True).to(self.device) #if args.use_window_normalization else None

    def train_forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, train_val=None):

        # return [Batch, Output length, Channel]

        if True: #self.args.use_window_normalization:
        ...
        outputs = self.rev(outputs, 'denorm') #if self.args.use_window_normalization else outputs

        f_dim = -1 if self.args.features == 'MS' else 0
        outputs = outputs[:, -self.pred_len:, f_dim:]
        ground_truth = x_dec[:, -self.pred_len:, f_dim:].to(self.device)
        
        # train_forward not part of MR-DDPM for now
        if not self.args.inspect_backward_pass is None:
            raise NotImplementedError

        if self.args.opt_loss_type == "mse":
...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.args = args
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu") #args.device

        self.seq_len = args.seq_len
        self.label_len = args.label_len
        self.pred_len = args.pred_len
        
        self.num_vars = args.enc_in

        self.smoothed_factors = [int(x) for x in args.moving_avg[1:-1].split(',')] 
        self.linear_history_len = args.seq_len # args.linear_history_lens
        
        self.num_bridges = len(self.smoothed_factors) + 1
        
        self.base_models = nn.ModuleList([BaseMapping(args, self.device, seq_len=self.linear_history_len, pred_len=self.pred_len) \
                                                for i in range(self.num_bridges)])
        self.decompsitions = nn.ModuleList([series_decomp(i) for i in self.smoothed_factors])
        
        if True: #args.u_net_type == "v0":
            self.u_nets = nn.ModuleList([My_DiffusionUnet_v0(args, self.device, self.num_vars, self.seq_len, 
                                            self.pred_len, net_id=i, smoothed_factors=self.smoothed_factors) for i in range(self.num_bridges)])
         
        self.diffusion_workers = nn.ModuleList([Diffusion_Worker(args, self.device, self.u_nets[i]) for i in range(self.num_bridges)])

        self.rev = RevIN(args, args.enc_in, affine=False, subtract_last=True).to(self.device) #if args.use_window_normalization else None
        
        args.type_sampler = "dpm"
        ...
        for i in range(self.num_bridges):
            if i == j:
                if i == 0:
                    ...
                    future_trend_i = self.rev(future_trend_i, 'denorm') #if self.args.use_window_normalization else future_trend_i
                    past_trend_i = self.rev(past_trend_i, 'denorm') #if self.args.use_window_normalization else past_trend_i
    ...
    def train_forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, return_mean=True, meta_weights=None, train_val=False):
        ...
        for i in range(self.num_bridges):
            if i == 0:
                ...
                linear_guess = self.rev(linear_guess, 'test_norm') #if self.args.use_window_normalization else linear_guess
            else:
                future_trend_i = future_trends[i-1]
                if False: #self.args.use_X0_in_THiDi:
                    past_trend_i = x_past
                else:
                    past_trend_i = past_trends[i-1]
                future_trend_i = self.rev(future_trend_i, 'denorm') #if self.args.use_window_normalization else future_trend_i
                ...
                linear_guess_i = self.rev(linear_guess_i, 'test_norm') #if self.args.use_window_normalization else linear_guess_i
    ...
    def test_forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        all_outs = []
        for i in range(1): #self.args.sample_times):
            ...
            outs_i = self.rev(outs_i, 'denorm') #if self.args.use_window_normalization else outs_i
...
if __name__ == "__main__":
    ...
    class Config():
        ...
        inspect_backward_pass = None #"forward"

    config = Config()

    net = Model(config).to(config.device)
    
    x = torch.ones((1, 720, 7)).to(config.device)
    x_mark_enc = torch.ones((1, 720, 4)).to(config.device)
    x_dec = torch.ones((1, 720, 7)).to(config.device)
    x_mark_dec = torch.ones((1, 720, 4)).to(config.device)

    print ("loss =", net.train_forward(x, x_mark_enc, x_dec, x_mark_dec))
    print ("forecast =", net.test_forward(x, x_mark_enc, x_dec, x_mark_dec).shape)
    exit()

    net.args.inspect_backward_pass = "forward"
    for idx in range(config.pred_len+1):
        for jdx in range(len(net.diffusion_workers)):
            net.diffusion_workers[jdx].set_backward_pass_inspect_timestep(idx)
        print ("loss =", net.train_forward(x, x_mark_enc, x_dec, x_mark_dec))
